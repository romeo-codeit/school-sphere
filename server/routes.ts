...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/cbt/exams', auth, async (req, res) => {
    try {
      ...
      if (limitParam === 'all') {
        fetchAll = true;
        limit = 100; // will iterate with cursor to fetch all
      } else {
        limit = Math.max(1, Math.min(100, parseInt(limitParam) || 10));
      }
      const offset = Math.max(0, parseInt(req.query.offset as string) || 0);
      const withQuestions = req.query.withQuestions !== 'false'; // default true
      const typeFilter = req.query.type ? String(req.query.type).toLowerCase() : undefined;
      ...
      if (fetchAll) {
        ...
        if (typeFilter) {
          exams = exams.filter((e: any) => String(e.type || '').toLowerCase() === typeFilter);
          total = exams.length;
        }
      }
