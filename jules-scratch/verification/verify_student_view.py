...
def run(playwright):
    ...
    try:
        ...
        page.get_by_test_id("link-attendance").click()
        expect(page).to_have_url(re.compile(".*attendance"), timeout=10000)

        # 3. Verify attendance history is displayed
        expect(page.get_by_text("My Attendance")).to_be_visible()
        expect(page.get_by_role("cell", name="Status")).to_be_visible()

        # 4. Take a screenshot
        page.screenshot(path="jules-scratch/verification/student-attendance-view.png")
